...
class WarehouseServiceImpl {
    ...
    async getBySameAsTitle(title) {
        ...
        return successResponse('Here are what I found:', this.#toBotRespondFormat(responses), false);
    }
    ...
    #toBotRespondFormat(slides) {
        ...
        for (let i = 0; i <= slides.length - 1; i += 1) {
            stringResponse = stringResponse.concat(`  ${i >= 0 && i < 9 ? `0${i + 1}` : i + 1}. `, `${slides[i].title}:  ${slides[i].value}\n`);
        }
